...
def get_employees():
    ...
    try:
        conn = psycopg2.connect(**DB_CONFIG) #Замените на ваши данные
        cur = conn.cursor()
        cur.execute("SELECT employee_id, first_name || ' ' || last_name FROM employees WHERE verify = 1") # выводим ФИО
...
def add_booking(animal_id, service_id, check_in_date, check_out_date, employee_id, food_id):
  """Добавляет бронирование в базу данных."""
  try:
      conn = psycopg2.connect(**DB_CONFIG) #Замените на ваши данные
      cur = conn.cursor()
      cur.execute("""
          INSERT INTO bookings (animal_id, service_id, check_in_date, check_out_date, status, employee_id, food_id)
          VALUES (%s, %s, %s, %s, 'Waiting', %s, %s)
      """, (animal_id, service_id, check_in_date, check_out_date, employee_id, food_id))
      conn.commit()
      cur.close()
      conn.close()
