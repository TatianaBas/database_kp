...
def registration():
    ...
    user_type_in = st.radio("Выберите тип пользователя:", ('Пользователь', 'Сотрудник'))
    if(user_type_in == 'Пользователь'):
        user_type = 'user'
    else:
        user_type = 'employee'
        ...
        if username and password and confirm_password:
            if password != confirm_password:
                ...
                if check_new_user_login(username):
                    ...
                    reg_new_user(username, hash_password(password), user_type)
